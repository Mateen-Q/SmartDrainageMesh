...
The system is built on **Contiki-NG**, utilizing the **NullNet** networking stack for lean, high-efficiency communication. Validation was performed using the **Cooja Simulator** with a 10-node sensor grid and a central aggregating gateway.

---

## **Key Features**
*   **TEEN Protocol Implementation:** Utilizes a dual-threshold reactive logic to minimize radio duty cycle.
*   **Spatial Data Aggregation:** The Sink node processes incoming alerts to isolate and report only the maximum detected flood levels.
*   **Resilient Mesh Topology:** Modeled using the Unit Disk Graph Medium (UDGM) to simulate realistic urban radio signal loss.
*   **Modular C Firmware:** Clean separation of concerns between sensing logic and network communication.
...
*   **Hard Threshold ($H_T = 30\text{cm}$):** Nodes remain silent until the water level exceeds this critical limit.
*   **Soft Threshold ($S_T = 2\text{cm}$):** Once an alert state is active, new data is only transmitted if the water level shifts by $\ge 2\text{cm}$.
...
*   Only "New Aggregated Max" events are logged for external emergency triggers, reducing redundant cloud traffic.
...
*   **/src**: Contains the Contiki-NG source code (`flood-sensor.c`, `sink-node.c`) and the project `Makefile`.
*   **/simulation**: The `Flood_Detection.csc` file for reproducing the 11-mote environment in Cooja.
...
2.  **Compile Firmware:**
    ...
    ```
3.  **Launch Cooja:**
    Open the `simulation/Flood_Detection.csc` file within the Cooja GUI.
4.  **Analyze Logs:**
    Observe the **Mote Output** to see the TEEN logic in action and the Sink node’s aggregation logs.
